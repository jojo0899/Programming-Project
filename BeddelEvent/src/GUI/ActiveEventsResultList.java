...
public class ActiveEventsResultList extends JFrame {
	...
	public ActiveEventsResultList() throws ParseException {
		...
		String query = "SELECT e.id, e.sportart, e.Datum, e.Uhrzeit, e.Postleitzahl, e.Stadt, e.Straße, e.Hausnummer, e.Anzahlplätze, e.kosten FROM event e join participate_on p on(e.id = p.eventid) WHERE username = '" + User.username +"' OR (username = veranstalter)";
